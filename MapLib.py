...
# -*- coding: UTF-8 -*-
"""
Autores: Carla Pascual Real, Pablo Noel Carreras Aguerri, Isaac Valdivia Lopez
"""
...
class Map2D:
    ...
    def obtain_neighbor_cells(self, cell):
        """ Devuelve los vecinos a la celda cell y su tipo de vecino, sin salirse del rango del mapa """
        neighbors = []

        x = cell[0] 
        y = cell[1]
        if x > 0:
            neighbors.append(((x - 1, y), 6))
        if x < (self.sizeX - 1):
    ...
    def fillCostMatrix(self, x_end, y_end):
        ...
        """
        self.costMatrix = np.ones( (self.sizeX, self.sizeY) ) * -2

        self.costMatrix[x_end, y_end] = 0

        cells_to_visit = [((x_end, y_end), 1)]

        while len(cells_to_visit) > 0:
            # Obtener celda a visitar
            cur_cell, cur_distance = cells_to_visit.pop(0)

            # Obtener vecinos de la celda
            cells_vecinas = self.obtain_neighbor_cells(cur_cell)
            for cell_vecina, tipo_vecino in cells_vecinas:
                if self.costMatrix[cell_vecina[0]][cell_vecina[1]] == -2:
                    if self.isConnected(cur_cell[0], cur_cell[1], tipo_vecino):
                        cells_to_visit.append((cell_vecina, cur_distance + 1))
                        self.costMatrix[cell_vecina[0]][cell_vecina[1]] = cur_distance

    def planPath(self, x_ini,  y_ini, x_end, y_end):
        """ 
        Rellena la matriz de costes y setea self.currentPath al 2d-array de cells a seguir
        para llegar al goal deseado
        """
        ...
        # Empieza en inicio, goal es el final
        path = [((x_ini, y_ini), -1)]
        cur_cell = (x_ini, y_ini)
        goal = (x_end, y_end)

        while cur_cell != goal:
            # Obtenemos las celdas vecinas, respetando limites del mapa
            cells_vecinas = self.obtain_neighbor_cells(cur_cell)

            # Filtramos las celdas vecinas, guardando solo aquellas conectadas a la celda actual

            cells_filtradas = [ (cell, tipo_vecino) for cell, tipo_vecino in cells_vecinas if self.isConnected(cur_cell[0], cur_cell[1], tipo_vecino) ]

            # Obtenemos los costes de las celdas candidatas
            costs = [ self.costMatrix[cell[0]][cell[1]] for cell, _ in cells_filtradas ]

            # Nos quedamos con la celda de menor coste
            next_cell_idx = np.argmin(costs)
            cur_cell = cells_filtradas[next_cell_idx][0]

            # Concatenamos nueva celda, al subarray de x, y al de las ys
            path.append(cells_filtradas[next_cell_idx])
    ...
    def replanPath(self, tipo_vecino, x_ini, y_ini, x_goal, y_goal):
        # quitar conexion
        self.deleteConnection(x_ini, y_ini, tipo_vecino)

        # replanificar camino
        self.planPath(x_ini, y_ini, x_goal, y_goal)
